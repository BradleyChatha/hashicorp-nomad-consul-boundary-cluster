resource "local_file" "inventory" {
  ...
  content  = <<HERE
[servers]
%{for server in module.nomad_consul_bootstrap_servers.servers}
${server.public_ip} ansible_connection=ssh ansible_user=admin ansible_ssh_private_key_file=${replace(module.nomad_consul_bootstrap_servers.ssh_key_filename, "../../", "")} 
%{endfor}
[bastion]
